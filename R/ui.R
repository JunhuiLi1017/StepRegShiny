...
#'
#' Creates the user interface for the StepReg Shiny application.
#' This function defines the complete UI structure including data upload,
#' stepwise regression configuration, and results visualization.
#'
#' @return A Shiny UI object
#'
#' @importFrom shiny tagList tags navbarPage tabPanel sidebarLayout sidebarPanel mainPanel selectInput fileInput checkboxInput radioButtons uiOutput actionButton tabsetPanel div verbatimTextOutput fluidPage fluidRow column downloadButton textAreaInput conditionalPanel sliderInput htmlOutput plotOutput navbarMenu
          ...
          mainPanel(
            tabsetPanel(
              ...
              tabPanel(
                ...
                conditionalPanel(
                  condition = "input.run_analysis",
                  htmlOutput("modelParametersText")
                ),
                withSpinner(DT::dataTableOutput("modelParameters")),
                conditionalPanel(
                  condition = "input.run_analysis",
                  htmlOutput("modelVariablesText")
                ),
                withSpinner(DT::dataTableOutput("modelVariables"))
              ),
              ...
              tabPanel(
                ...
                conditionalPanel(
                  condition = "input.run_analysis",
                  htmlOutput("modelPerformanceText")
                ),
                withSpinner(DT::dataTableOutput("modelPerformance"))
              )
            )
          )
        )
        ...
        tabPanel(
          ...
          tags$div(
            ...
            tags$ul(
              ...
              tags$li("Model performance evaluation across different strategies and metrics")
            )
          )
        ),
        tabPanel(
